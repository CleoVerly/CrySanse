      ...
      <div className={`flex ${reverse ? "flex-col-reverse md:flex-row" : "flex-col md:flex-row-reverse"} items-center gap-8 py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6`}>
        ...
        <div className="w-full md:w-1/2 mt-4 md:mt-0">
          <MotionWrapper variant="fade-left">
            <h2 className="mb-2 text-2xl md:text-4xl font-extrabold text-primary-dark font-poppins">{title}</h2>
            <p className="mb-6 text-base font-light text-primary-darkest font-open-sans md:text-lg">{description}</p>

            <div className="space-y-4">
              {features.map((item, index) => (
                <div key={index} className="flex items-start gap-2">
                  <img src={checkIcon} alt="check" className="w-5 h-5 mt-1" />
                  <p className="text-base md:text-md text-primary-darkest font-open-sans">{item}</p>
                </div>
              ))}
            </div>
