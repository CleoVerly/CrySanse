...
const beratBadan = require("../models/beratModel.js");

const API_KEY = process.env.API_KEY;

const genAI = new GoogleGenerativeAI(API_KEY);
exports.getBeratBadan = async (req, res) => {
  try {
    const fetchDataBeratBadan = await beratBadan.find();
    if (fetchDataBeratBadan.length === 0) {
      res.status(404).json({ message: "Data Berat Tidak Ditemukan" });
    } else {
      res.status(200).json({ fetchDataBeratBadan });
    }
  } catch (error) {
...
exports.postBeratBadan = async (req, res) => {
...
exports.getIdBeratBadan = async (req, res) => {
  try {
    const fetchDataBeratBadan = await beratBadan.findOne({
      _id: req.params.id,
    });

    if (!fetchDataBeratBadan) {
      res.status(404).json({ message: "Data Berat Tidak Ditemukan" });
    } else {
      res.status(200).json({ fetchDataBeratBadan });
    }
